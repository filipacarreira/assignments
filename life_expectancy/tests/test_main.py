...
def test_main_csv(pt_life_expectancy_expected):
    ...
    pt_life_expectancy_actual = main(EU_CSV_PATH, PT_EXPECTED_PATH, Region.PT).reset_index(drop = True)
...
def test_main_json(pt_life_expectancy_expected):
    ...
    pt_life_expectancy_actual = main(EU_ZIP_PATH, PT_EXPECTED_PATH, Region.PT).reset_index(drop = True)
